...
getFilesFromCourse :: C.CourseID -> Global [FileJ]
getFilesFromCourse id' = simpleHttpJSON $ "courses/" ++ show id' ++ "/files"

downloadFromCourse :: FilePath -> C.CourseID -> Global DownloadState
...
downloadToNewFolder :: FilePath -> C.CourseID -> C.CourseShortName -> Global ()
downloadToNewFolder downloadDir id' nm = do
    lift $ putStrLn $ "Now downloading for course " ++ nm
    catchE' $ createDirectoryIfMissing False $ downloadDir </> nm
